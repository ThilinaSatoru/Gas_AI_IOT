  ...
  private:
    ...
    float getStableReading(int numReadings, int delayBetweenReadings) {
      ...
      for (int i = 0; i < numReadings; i++) {
        int attempts = 0;
        while (attempts < 10) { // Max 10 attempts per reading
          if (_loadCell.update()) {
      ...
      for (int i = 0; i < validReadings; i++) {
        ...
        if (deviation < abs(median) * 0.1) { // Within 10% of median
          sum += readings[i];
          filteredCount++;
        }
      }
    ...
    HX711_Data readHX711() {
      ...
      if (data.isValid && (millis() - lastPrint > 1000)) { // Print every 1 second
  ...
  public:
    ...
    void continuousWeightUpdate() {
      ...
      if (_loadCell.update()) {
        ...
        if (abs(weightGrams) > 50000) { // 50kg limit
          consecutiveFailures++;
        ...
        if (lastValidTime > 0 && (currentTime - lastValidTime) < 100) { // Less than 100ms
          float rateOfChange = abs(weightGrams - lastValidWeight) / ((currentTime - lastValidTime) / 1000.0);
          if (rateOfChange > 10000) { // More than 10kg/second is suspicious
...
void sensorReadTask(void *parameter) {
  ...
  for(;;) {
    ...
    if (++printCounter >= 10) { // Every 100ms (10 * 10ms)
      printCounter = 0;
  ...
  for(;;) {
    if (app.ready()) {
      ...
      // MQ7
      {
        ...
        if (co_value > 1000000) co_value = 1000000;
        mq7Data["co"] = co_value;
